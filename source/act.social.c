...
*/ 
    ...
    {
	"airguitar", POSITION_RESTING, 0, 0,
    ...
    {
	"apologize", POSITION_RESTING, 0, 0,
    ...
    {
	"cannonball", POSITION_RESTING, 0, 0,
    ...
    {
	"marvelous", POSITION_RESTING, 0, 0,
    ...
    {
    "panic", POSITION_RESTING, 0, 0,
    "PANIc! You try to escape..",
	"$n panics, and attempts to flee.",
    "PANIc! You try to escape from $N.",
	"$n tries to flee from you, but is too exhausted!"
	"$n tries to flee from $N, but is too exhausted!"
    "PANIc! You try to flee from yourself... *too sad*.",
    "$n tries to flee from $mself. You feel sorry for $m."
    },
    
    {
	"pat", POSITION_RESTING, 0, 1,
	"Pat whom?",
	NULL,
	"You pat $N on $S head.",
	"$n pats $N on $S head.",
    ...
    {   
	"renandstimpy", POSITION_RESTING, 0, 0,
	...
	{
	"scratch", POSITION_RESTING, 0, 0,
	"You scratch yourself.",
	"$n feverishly scratches $mself.",
	"You scratch $M.",
	"$n busily scratches $N.",
	"$n scratches you.",
	"You scratch yourself.",
	"$n scratches $mself."
	},
    ...
    {
	"snowball", POSITION_STANDING, 0, 0,
...
#define MAX_SOCIALS	 226

int check_social( charType *ch, char * command, char * argument )
{
    charType *	victim;
    ...
    for( cmd = 0; social_table[cmd].name[0] != '\0'; cmd++ )
    {
		if( command[0] == social_table[cmd].name[0]
	    	&& isprefix( command, social_table[cmd].name ) )
		{
	    	found = TRUE;
	    	break;
		}
    ...
    {
    	case POSITION_DEAD:
			sendf( ch, "Lie still; you are DEAD." ); return TRUE;

    	case POSITION_INCAP:
    	case POSITION_MORTALLYW:
			sendf( ch, "You are hurt far too badly for that." ); return TRUE;

    	case POSITION_STUNNED:
			sendf( ch, "You are too stunned to do that." ); return TRUE;
    	...
    	case POSITION_SLEEPING:
	 		...
	 		*/
			if ( !stricmp( social_table[cmd].name, "snore" ) )
	    		break;
			sendf( ch, "In your dreams, or what?" ); return TRUE;
    }
    ...
    if( arg[0] == '\0' )
    {
		act( social_table[cmd].char_no_arg,   1, ch, NULL, victim, TO_CHAR    );
		act( social_table[cmd].others_no_arg, 1, ch, NULL, victim, TO_ROOM    );
    }
    else if( !( victim = find_char_room( ch, arg ) ) )
    {
		sendf( ch, "They aren't here." );
    }
    else if( victim == ch )
    {
		act( social_table[cmd].char_auto,     1, ch, NULL, victim, TO_CHAR    );
		act( social_table[cmd].others_auto,   1, ch, NULL, victim, TO_ROOM    );
    }
    else
    {
		act( social_table[cmd].char_found,    1, ch, NULL, victim, TO_CHAR    );
		act( social_table[cmd].vict_found,    1, ch, NULL, victim, TO_VICT    );
		act( social_table[cmd].others_found,  1, ch, NULL, victim, TO_NOTVICT );

		if( IS_NPC( victim ) && !IS_NPC( ch ) && ch->level < IMO )
		{
	      	if( !IS_AFFECTED( victim, AFF_CHARM ) && AWAKE( victim ) )
			{
				if( social_table[cmd].insult && number( 1, 3 ) == 1 )
				{
					sendzf( 0, world[victim->in_room].zone, 
                            "%s yells, '%s. How dare you do that to me? Charge!!'", 
							GET_NAME( victim ), GET_NAME( ch ) );
					hit( victim, ch, TYPE_UNDEFINED );
					return 1;
				}
				if( social_table[cmd].sex && number( 1, 3 ) == 1 )
				{
					if( GET_SEX( victim ) == SEX_FEMALE && (GET_SEX( victim ) != GET_SEX( ch )) )
					{
						sendzf( 0, world[victim->in_room].zone, "%s yells, '%s. Rogue!!'", 
							GET_NAME( victim ), GET_NAME( ch ) );
						hit( victim, ch, TYPE_UNDEFINED );
					}
					return 1;
	    		...
	    		switch ( number( 0, 12 ) )
	    		{
				case 0: if( ch->level < victim->level )
						{
						sendzf( 0, world[victim->in_room].zone, 
							"%s yells, '%s. Are you insulting me?!?'", 
								GET_NAME( victim ), GET_NAME( ch ) );
						hit( victim, ch, TYPE_UNDEFINED );
						}
						break;
	    		case 1: case 2: case 3: case 4:
						i = number( 0, MAX_SOCIALS );
						check_social( victim, social_table[i].name, ch->name );
						break;	
	    		case 5: case 6: case 7: case 8:
					act( social_table[cmd].char_found, 1, victim, NULL, ch, TO_CHAR    );
					act( social_table[cmd].vict_found, 1, victim, NULL, ch, TO_VICT    );
					act( social_table[cmd].others_found, 1, victim, NULL, ch, TO_NOTVICT );
					break;

	    		case 9: case 10: case 11: case 12:
					act( "You slap $N.",  1, victim, NULL, ch, TO_CHAR    );
					act( "$n slaps you.", 1, victim, NULL, ch, TO_VICT    );
					act( "$n slaps $N.",  1, victim, NULL, ch, TO_NOTVICT );
					break;
	    		}
			}
		}
...
void do_socials( charType * ch, char * argument, int cmd )
{
	int			i, mod;
	char		buf[256];

	for( buf[0] = 0, i = 0, mod = 0; social_table[i].name[0]; i++ )
	{
		sprintf( buf + strlen( buf ), "%12s ", social_table[i].name );

		if( ++mod >= 6 )
		{
			mod = 0; sendf( ch, "%s", buf ); buf[0] = 0;
		}
	}
}

void do_insult( charType * ch, char * argument, int cmd )
{
   	char 		arg[ MAX_INPUT_LENGTH ];
  	charType *	victim;
  	...
  	if( *arg ) 
	{
    	if( !(victim = find_char_room(ch, arg)) ) 
		{
      		sendf( ch, "Can't hear you!" );
    	} 
		else 
		{
      		if( victim != ch ) 
			{
        		sendf( ch, "You insult %s.", GET_NAME(victim) );
        		switch( number(0, 3) ) 
				{
          		case 0 :
			...
			else sendf( ch, "You feel insulted." );
      	}
  	} 
	else sendf( ch, "Sure you don't want to insult everybody." );
}
