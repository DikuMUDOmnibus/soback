...
int is_allow_kill( charType * ch, charType * vict )
{
	if( OMNI( vict ) ) return 0;
	if( ch == vict ) return 0;
	if( nokillflag ) if( !IS_NPC(ch) && !IS_NPC(ch) ) return 0;
	if( is_same_group( ch, vict ) && !IS_NPC(ch) && !IS_NPC(vict) ) return 0;
		...
		else 
		{
      		...
      		for( k = ch->followers; k; k = k->next ) 
	  		{
        		if( org_room == k->follower->in_room )
		  			...
		  			{
            			found = TRUE;
            			interpreter( k->follower, message, 1 );
          			}
      		}
...
void do_tornado( charType *ch, char * argument, int cmd )
{
  	...
  	for( tch=world[ch->in_room].people; tch; tch = tch2) 
  	{
    	...
    	if( is_allow_kill( ch, tch ) && ch->move > 0 ) 
		{
